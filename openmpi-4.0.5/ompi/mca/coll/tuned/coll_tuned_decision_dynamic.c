...
#include <libinc_coll.h>

#define MCA_COLL_BASE_TAG_NEWREDUCE 0xff3344f4
#define MCA_COLL_BASE_TAG_NEWBARRIER 0xff3344f3
...
int ompi_inc_multicore_data_reduce(const void *sendbuf, void *recvbuf, int count, ompi_datatype_t* datatype, ompi_op_t* op, int root, ompi_communicator_t* comm, mca_coll_base_module_t *module, uint32_t segsize,int max_outstanding_reqs){
    int err = ompi_coll_base_reduce_intra_binomial(sendbuf, recvbuf, count, datatype, op, root, comm, module, segsize, max_outstanding_reqs);
    return err;
}

int ompi_inc_multicore_data_bcast(void* buffer, int count, struct ompi_datatype_t* datatype, int root, struct ompi_communicator_t* comm, mca_coll_base_module_t *module, uint32_t segsize){
    int err = ompi_coll_base_bcast_intra_binomial(buffer,count,datatype,root,comm,module,segsize);
    return err;
}
 ...
 */
int ompi_coll_tuned_allreduce_intra_dec_dynamic (const void *sbuf, void *rbuf, int count,
                                             ...
                                             mca_coll_base_module_t *module)
{
    ...
    if (tuned_module->user_forced[ALLREDUCE].algorithm) {
        if((tuned_module->user_forced[ALLREDUCE].algorithm == 8 || tuned_module->user_forced[ALLREDUCE].algorithm == 9)) {
            if((count * dtype->super.size > 256)) return ompi_coll_tuned_allreduce_intra_dec_fixed (sbuf, rbuf, count, dtype, op, comm, module);
	    ...
	    int comm_rank = ompi_comm_rank(comm);
            ...
            comm->INC_socket_num = 1;
            if ((tuned_module->user_forced[ALLREDUCE].algorithm == 9) && (server_rank_num > 1)){
                comm->INC_socket_num = 2;
                if (comm->INC_socket_num >= 2) {
                    server_rank_num /= comm->INC_socket_num;
                    server_num *= comm->INC_socket_num;
                }
            }
            if (comm->INC_switch_info_got == 0 || comm->INC_comm_id == 0) {
                ompi_inc_control_surface(comm, comm_rank, comm_size, server_rank_num, server_num, comm->INC_socket_num);
               int cnt = 0;
            }
            if(comm->newcomm != NULL){
                int newcomm_rank = ompi_comm_rank(comm->newcomm);
           	int newcomm_size = ompi_comm_size(comm->newcomm);
            }
            if(!comm->INC_feature_available){
                return ompi_coll_tuned_allreduce_intra_dec_fixed(sbuf, rbuf, count, dtype, op, comm, module);
            }
	       if(((comm->inc_sw_feature.sup_allreduce_op>>(op->op_type-1) & 0x0001) == 0) || ((comm->inc_sw_feature.sup_data_type>>(dtype->id) & 0x0001) == 0)){
		        return ompi_coll_tuned_allreduce_intra_dec_fixed(sbuf, rbuf, count, dtype, op, comm, module);
            }
            if (comm->INC_feature_available && (count * dtype->super.size <= 256)) { // Data surface

		        if(server_rank_num > 1){
                    ptrdiff_t dsize, gap = 0;
                    dsize = opal_datatype_span(&dtype->super, count, &gap);
                    char * temprbuf_free = (char*) malloc(dsize);
                    void *ssbuf = temprbuf_free - gap;
		            int err_reduce = mca_coll_basic_reduce_log_intra(sbuf, ssbuf, count, dtype, op, 0, comm->newcomm, comm->newcomm->c_coll->coll_reduce_module);
                    if(err_reduce != MPI_SUCCESS){
                        printf("multi core reduce failed!\n");
                    }
                    if(comm_rank % server_rank_num == 0){
                        ompi_inc_data_surface(ssbuf, rbuf, count, dtype, op, comm, comm_rank, server_rank_num);
                    }
                    int err_bcast = mca_coll_basic_bcast_log_intra(rbuf, count, dtype, 0, comm->newcomm, comm->c_coll->coll_bcast_module);
                    if(err_bcast != MPI_SUCCESS){
                        printf("multi core bcast failed!\n");
                    }
                    free(temprbuf_free);
                }else{
                    ompi_inc_data_surface(sbuf, rbuf, count, dtype, op, comm, comm_rank, server_rank_num); // Call INC Data surface
                }
                return 0;
            } else {
                return ompi_coll_tuned_allreduce_intra_dec_fixed(sbuf, rbuf, count, dtype, op, comm, module);
    ...
    int ret = ompi_coll_tuned_allreduce_intra_dec_fixed (sbuf, rbuf, count, dtype, op, comm, module);
    return ret;
}
